...
#include <heiman/convertimagefromHTPApublishedConfig.h>
...
void config_callback(heiman::convertimagefromHTPApublishedConfig &config, uint32_t level) {
  if (respect_launch_parameters)
    {
      // solo se inicializan los parámetros una vez
      //if (!ros::param::get("~/zoom", zoom)) zoom = config.zoom;  
      if (!ros::param::get("~/lower_limit", lower_limit)) lower_limit = config.lower_limit;  
      if (!ros::param::get("~/upper_limit", upper_limit)) upper_limit = config.upper_limit;  	
